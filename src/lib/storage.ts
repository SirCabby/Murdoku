 ...
 * key tab.
 */
function upgradeV13(old: LibraryV13): LibraryV14 {
  const puzzles: Record<string, PuzzleV14> = {}
  for (const [id, p] of Object.entries(old.puzzles)) {
...
/**
 * Add a `solved` badge (starting `false`) to every puzzle, taking v14 to v15.
 * The badge is play-completion state the author never sets, so every migrated
 ...
 */
function upgradeV15(old: LibraryV15): LibraryV16 {
  const puzzles: Record<string, PuzzleV16> = {}
  for (const [id, p] of Object.entries(old.puzzles)) {
    puzzles[id] = { ...p, clues: [] }
  }
...
/**
 * Add an empty `doors` map to every puzzle, taking v16 to v17. Doors are
 * authoring content that predates this version, so every migrated puzzle starts
...
/** A version-14 puzzle — has an answer-key murderer but no solved badge yet. */
interface PuzzleV14 extends PuzzleV13 {
  solutionMurderer: string | null
}

interface LibraryV14 {
  version: 14
  folders: Folder[]
...
/** A version-16 puzzle — has room clues but no doors yet. */
interface PuzzleV16 extends PuzzleV15 {
  clues: Clue[]
}

interface LibraryV16 {
  version: 16
  folders: Folder[]
...
function seedLibrary(): Library {
  ...
  // A door on the interior wall dividing the fenced room from the parlor,
  // showing the door feature out of the box.
  ...
  return {
    ...
    puzzles: {
      [puzzleId]: {
        ...
        solution,
        solutionMurderer,
        // A partially-worked demo board, not a finished solve — starts unsolved.
        solved: false,
        createdAt: 0,
        updatedAt: 0,
      },
